# Frontend Mentor - IP address tracker solution

This is a solution to the [IP address tracker challenge on Frontend Mentor](https://www.frontendmentor.io/challenges/ip-address-tracker-I8-0yYAH0). Frontend Mentor challenges help you improve your coding skills by building realistic projects. 

Difficulty: Level 3 (Intermediate)

## Overview

### The challenge

Users should be able to:

- View the optimal layout for each page depending on their device's screen size
- See hover states for all interactive elements on the page
- See their own IP address on the map on the initial page load
- Search for any IP addresses or domains and see the key information and location

### Links

- Live Site URL: [https://jordansgit.github.io/ip-address-tracker/](https://jordansgit.github.io/ip-address-tracker/)

### Built with

- HTML5
- CSS 
- JavaScript 
- Leaflet.JS 

### Screenshots

![Desktop Screenshot](./screenshot.jpg)
![Tablet Screenshot](./screenshot.jpg)
![Mobile Screenshot](./screenshot.jpg)

### Comments 

I had to use a different API, because the 1 provided in this design spec requires you to pay to get lon & lat data. 
I used 2 API's. 
- 1 to get users IP address 
- 1 to get key info of IP address. 
--- the timezone data looks different because of this. 

For user's IP info, I haven't yet included a failure scenario if user doesn't give access to location access. 

Since I started this project ages ago, I didn't worry about the css of making it responsive, main focus was practicing JS skills. 